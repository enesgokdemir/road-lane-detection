...
def draw_lines(image, hough_lines):
    for line in hough_lines:
        x1, y1, x2, y2 = line[0]
        cv2.line(image, (x1, y1), (x2, y2), (100, 250, 0), 2)
...
def process(img):
    ...
    canny = cv2.Canny(gray_img, 180, 200)

    roi_img = roi(canny, np.array([roi_vertices], np.int32))

    lines = cv2.HoughLinesP(roi_img, 1, np.pi / 180, threshold=15, minLineLength=15, maxLineGap=15)
